...
DON_vect <- seq(from = 0, to = 0.7, length.out = 50)
NH3_DON_vect <- NULL

for (riverDON in DON_vect){
  model_output <- sens(riverDON, "riverDON")
  conc_200 <- (model_output[200] + model_output[201]) / 2
  
  NH3_DON_vect <- c(NH3_DON_vect, conc_200)
}
...
Server.Aquifer <- shinyServer(function(input, output, session) {
  ...
  output$PlotAquifer <- renderPlot({
    ...
    for (i in 1:length(plt_variables)) {
      ...
      if (plt_variables[i] %in% c("DON", "Nitrate", "Ammonia") & input$wellLocation) {
         abline(v = 200, col = 6)
      } 
    }
